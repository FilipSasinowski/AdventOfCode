...
/*
 ...
 We really need to come up with a better naming scheme. T
 ell you what: if you can help me with something quick,
...
class Lottery {
    ...
    init(puzzleInput: [String]?) {
        self.cards = Lottery.getCardsFrom(puzzleInput: puzzleInput)
    }
    
    struct Card {
        let id: Int
        var copies: Int
        let scrachtedNumbers: [Int]
        let winningNumbers: [Int]
        var matchingNumbers: [Int] {
            return winningNumbers.filter({ scrachtedNumbers.contains($0) })
        }
        var points: Int {
            return 2 ^^ (matchingNumbers.count - 1)
        }
    }
    private static func getCardsFrom(puzzleInput: [String]?) -> [Card] {
        guard let puzzleInput = puzzleInput else { return [] }
        let cards = puzzleInput.compactMap { line in
            var card: Card?
            guard
                let colonRange = line.range(of: ":"),
                let middlePartRange = line.range(of: "|")
            else {
                return card
            }
            let id = String(line[line.startIndex ..< colonRange.lowerBound]).calculateFirstNumber()
            let scrachtedNumbers = String(line[colonRange.upperBound ..< middlePartRange.lowerBound]).components(separatedBy: " ").compactMap({ Int($0) })
            let winningNumbers = String(line[middlePartRange.upperBound ..< line.endIndex]).components(separatedBy: " ").compactMap({ Int($0) })

            return Card(id: id, copies: 1, scrachtedNumbers: scrachtedNumbers, winningNumbers: winningNumbers)
        }
        return cards
    }
}
...
extension Lottery {
    func getSumOfAllGames() -> Int {
        return cards.reduce(into: 0, { x,y in x += y.points })
    }
}

assert(Lottery(puzzleInput: partOneTestInput).getSumOfAllGames() == 13)
...
extension Lottery {
    private func calculateCardCopies() {
        for card in cards {
            appendCopiesToCards(from: card.id, amount: card.matchingNumbers.count)
        }
    }
    
    func appendCopiesToCards(from id: Int, amount: Int) {
        let id = id - 1
        guard id != cards.count - 1 else { return }
        guard amount > 0 else { return }
        let copyMultipier = cards[max(0, id)].copies
        let endIndex = min(id + amount, cards.count - 1)
        for cardID in id + 1 ... endIndex {
            cards[cardID].copies += copyMultipier
        }
    }
    
    func sumAllScratchpads() -> Int {
        calculateCardCopies()
        return cards.reduce(into: 0, { x,y in x += y.copies })
    }
}
